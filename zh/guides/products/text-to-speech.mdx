...
`provider` **无需传入**。服务端根据 `data[].voiceId` 在系统/克隆音色库中查询并自动推断，再传给合成引擎。批内所有 `voiceId` 的 `provider` 必须一致。规则与错误说明见 [支持的克隆方式](/zh/guides/supported-clone-methods#文本转语音voiceid-与-provider) 与 [错误码](/zh/guides/errors)。
