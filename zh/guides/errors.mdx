...
<AccordionGroup>
  ...
  <Accordion title="402 配额不足">
    任务无法扣减项目积分时返回。见 [价格](/zh/guides/pricing) 或在控制台 <a href={productUrls.pricing}>充值</a>。
  </Accordion>
  <Accordion title="502 上游失败">
    上游服务调用失败或超时。使用 `taskId` 轮询 `GET /openapi/tasks/detail`（Dashboard **我的任务** 不包含 Open API 任务）；若已配置 Webhook，可等待重试。
  </Accordion>
</AccordionGroup>
