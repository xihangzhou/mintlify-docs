...
<AccordionGroup>
  ...
  <Accordion title="TTS voice validation (400)">
    ...
    - **Unknown voiceId** — `voiceId not found in system or clone voice library: data[0] voiceId=... Use POST /openapi/v1/assets/voice/basic/list for system voices or POST /openapi/v1/assets/voice/clone/list for clone voices.`
    - **Mixed providers in one batch** — `all voiceId in one batch must share the same provider, but found multiple: Microsoft, GC. Voice mapping: ... Please use voices from the same provider, or split into separate requests.`
    - **`timbreRefAudio` with system voice** — `timbreRefAudio requires a clone provider (V1-V5), but voiceId in this batch resolved to provider Microsoft. ...`
