...
<AccordionGroup>
  ...
  <Accordion title="502 Upstream failure">
    An upstream service failed or timed out. Poll `GET /openapi/tasks/detail` with the `taskId` (your task list on vmeg.ai does not include Open API jobs). If a webhook is configured, wait for retries.
  </Accordion>
</AccordionGroup>
