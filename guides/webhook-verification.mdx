...
2. When a task completes, VMEG POSTs the JSON body to your URL and attaches:
   ...
   - **`X-Signature`** — lowercase hex of `HMAC-SHA256(secret, X-Timestamp + rawBody)`
...
3. Hash the **raw request body bytes** — do not re-serialize JSON.
4. Compare your HMAC to `X-Signature` using a **constant-time** comparison.
